...
$link      = $args['archive'];
$loop      = $args['loop'];
$type      = preg_replace('/[^A-Za-z0-9]/', '', strtolower($title));
...
<section class="mb-6 mt-5 md:mt-0">
    ...
    <?php if (!$slider): ?>
    <section class="kira-grid-listing mx-auto px-2 md:px-0 w-full flex-auto">
        <?php get_template_part('template-parts/sections/listing/use', 'grid', [
    'animes' => $query->animes,
]);?>
    </section>
    <?php else: ?>
    <section class="mx-auto w-full flex-auto px-2 md:px-0 overflow-hidden">
        <div data-current-slider="<?php echo $type ?>" data-is-loop="<?php echo $loop ? '1' : '0'; ?>"
            class="swiper swiper-sections swiper-<?php echo $type; ?>">
            <div class="swiper-wrapper" style="min-width: 100vw;">
                <!-- Slides -->
                <?php get_template_part('template-parts/sections/listing/use', 'slider', [
    'animes' => $query->animes,
]);?>
            </div>
        </div>
