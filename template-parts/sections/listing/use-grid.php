...
<div class="w-full bg-gradient-to-t from-primary to-transparent rounded shadow shadow-primary">
    <div class="block relative w-full group kira-anime add-rem overflow-hidden">
        <?php if ($usestatus):
        $slug = !is_wp_error($status) && !empty($status) ? $status[0]->slug : null;
        if ($slug && 'completed' == strtolower($slug)):
        ?>
        <div class="status_show" style="background-color: var(--completed-status)"><?php
        echo $status[0]->name; ?></div>
        <?php endif;endif;?>
...
echo implode('/', $a);?>
            ...
            <span class="text-[11px] px-2 py-1 rounded-md font-medium h-[25px] text-text-accent bg-accent-3 ml-auto">
                E<?php echo $latest && $latest->meta['number'] ? $latest->meta['number'] : '?'; ?>
            </span>
            <?php endif;?>
        </div>
        <a <?php if (get_theme_mod('__show_tooltip', 'show') === 'show'): ?> data-tippy-trigger
            data-tippy-content-to="<?=$anime->anime_id?>" <?php endif;?> href="<?=$anime->url;?>"
...
if (function_exists('str_contains')) {
    $is_adult = str_contains($meta['rate'], '18');
} else {
    $is_adult = stripos($meta['rate'], '18') !== false;
}
?>
        <?php if (isset($meta['rate']) && $is_adult): ?>
        <span class="bg-orange-600 text-text-color text-xs font-semibold px-2 py-1 rounded-md absolute top-2 right-2">
            18+
        </span>
        <?php endif;?>
    </div>

    <div style="min-height: 4.906rem"
        ...
        <div class="text-xs text-text-color w-full line-clamp-1 absolute bottom-1 text-opacity-75">
            <span
                class="inline-block md:my-3 uppercase"><?=!is_wp_error($type) && count($type) > 0 ? $type[0]->name : 'TV';?></span>
            <span class="inline-block bg-gray-600 w-1 h-1 mx-2"></span>
            <span class="inline-block md:my-3"><?php echo isset($meta['duration']) ? $meta['duration'] : '24m'; ?>
            </span>
        </div>
    </div>
</div>
